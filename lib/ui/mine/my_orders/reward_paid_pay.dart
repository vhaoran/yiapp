...
class RewardPaidPay extends StatefulWidget {
  RewardPaidPay({Key key}) : super(key: key);

  @override
  _RewardPaidPayState createState() => _RewardPaidPayState();
}

class _RewardPaidPayState extends State<RewardPaidPay>
    with AutomaticKeepAliveClientMixin {
  var _future;
  int _pageNo = 0;
  int _rowsCount = 0;
  final int _rows_per_page = 10; // 默认每页查询个数
  List<BBSPrize> _l = []; // 悬赏帖已付款列表
  ...
  _fetch() async {
    ...
    try {
      ...
      Debug.log("总的悬赏帖已付款个数：$_rowsCount");
      l.forEach((src) {
        var dst = _l.firstWhere((e) => src.id == e.id, orElse: () => null);
        if (dst == null) _l.add(src);
      });
      if (mounted) setState(() {});
      Debug.log("当前已查询悬赏帖已付款个数：${_l.length}");
    } catch (e) {
      Debug.logError("分页查询悬赏帖已付款出现异常：$e");
    }
  }
