...
class MyMasterOrders extends StatefulWidget {
  MyMasterOrders({Key key}) : super(key: key);

  @override
  _MyMasterOrdersState createState() => _MyMasterOrdersState();
}

class _MyMasterOrdersState extends State<MyMasterOrders> {
  var _future;
  int _pageNo = 0;
  int _rowsCount = 0;
  final int _rows_per_page = 10; // 默认每页查询个数
  List<YiOrder> _l = []; // 大师未完成列表(前台)
  ...
  _fetch() async {
    ...
    try {
      ...
      Debug.log("总的大师未完成列表(前台)个数：$_rowsCount");
      l.forEach((src) {
        // 在原来的基础上继续添加新的数据
      ...
      setState(() {});
      Debug.log("当前已查询大师未完成列表(前台)个数：${_l.length}");
    } catch (e) {
      Debug.logError("分页查询大师未完成列表(前台)出现异常：$e");
    }
  }
