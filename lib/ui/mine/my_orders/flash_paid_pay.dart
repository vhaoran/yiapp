...
class FlashPaidPay extends StatefulWidget {
  FlashPaidPay({Key key}) : super(key: key);

  @override
  _FlashPaidPayState createState() => _FlashPaidPayState();
}

class _FlashPaidPayState extends State<FlashPaidPay>
    with AutomaticKeepAliveClientMixin {
  var _future;
  int _pageNo = 0;
  int _rowsCount = 0;
  final int _rows_per_page = 10; // 默认每页查询个数
  List<BBSVie> _l = []; // 闪断帖已付款列表
  ...
  _fetch() async {
    ...
    try {
      ...
      Debug.log("总的闪断帖已付款个数：$_rowsCount");
      l.forEach((src) {
        var dst = _l.firstWhere((e) => src.id == e.id, orElse: () => null);
        if (dst == null) _l.add(src);
      });
      if (mounted) setState(() {});
      Debug.log("当前已查询闪断帖已付款个数：${_l.length}");
    } catch (e) {
      Debug.logError("分页查询闪断帖已付款出现异常：$e");
    }
  }
