...
import 'package:yiapp/ui/masters/vie/master_vie_aim_page.dart';
...
class MasterVieAimMain extends StatefulWidget {
  MasterVieAimMain({Key key}) : super(key: key);

  @override
  _MasterVieAimMainState createState() => _MasterVieAimMainState();
}

class _MasterVieAimMainState extends State<MasterVieAimMain>
    with AutomaticKeepAliveClientMixin {
  var _future;
  int _pageNo = 0;
  int _rowsCount = 0;
  final int _rowsPerPage = 10; // 默认每页查询个数
  List<BBSVie> _l = []; // 闪断帖可抢单列表
      ...
      child: PostComCover(
        ...
        events: Row(
          children: <Widget>[
            ...
            PostComButton(text: "抢单", onPressed: () => _doAimVie(vie)),
          ],
        ),
      ),
    );
  ...
  /// 抢闪断帖
  void _doAimVie(BBSVie vie) async {
    try {
      BBSVie res = await ApiBBSVie.bbsVieAim(vie.id);
      if (res != null) {
  ...
  void _lookViePost(String postId) {
    CusRoute.push(context, MasterVieAimPage(postId: postId))
        .then((value) async {
      if (value != null) {
        await _refresh();
      }
