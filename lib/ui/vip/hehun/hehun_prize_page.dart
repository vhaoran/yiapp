...
// ------------------------------------------------------
// author：suxing
// date  ：2021/1/26 下午4:52
// usage ：合婚悬赏帖求测
// ------------------------------------------------------

class HeHunPrizePage extends StatefulWidget {
  final SubmitHeHunData heHunData;

  HeHunPrizePage({this.heHunData, Key key}) : super(key: key);

  @override
  _HeHunPrizePageState createState() => _HeHunPrizePageState();
}

class _HeHunPrizePageState extends State<HeHunPrizePage> {
  var _future;
  ...
  _fetch() async {
    try {
      ...
      Log.error("会员合婚测算获取运营商悬赏帖标准出现异常：$e");
    }
  }
  ...
  void _doPrizeOrder() async {
    try {
      BBSPrize prize = await ApiBBSPrize.bbsPrizeAdd(_heHunData.toJson());
      if (prize != null) {
        CusToast.toast(context, text: "下单成功", pos: ToastPos.bottom);
        BalancePay(
          context,
          data: PayData(amt: _heHunData.amt, b_type: b_bbs_prize, id: prize.id),
          onSuccess: () {
            CusRoute.push(
              context,
              UserPrizeDoingPage(
                postId: prize.id,
                backData: "发合婚悬赏帖后进入处理中页面，然后返回时携带数据",
              ),
            ).then((value) {
              if (value != null) {
