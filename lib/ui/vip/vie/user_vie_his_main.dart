...
class UserVieHisMain extends StatefulWidget {
  UserVieHisMain({Key key}) : super(key: key);

  @override
  _UserVieHisMainState createState() => _UserVieHisMainState();
}

class _UserVieHisMainState extends State<UserVieHisMain>
  ...
  _fetch() async {
    ...
    try {
      ...
      if (pb != null) {
        ...
        Log.info("已加载闪断帖已完成个数 ${_l.length}");
        setState(() {});
      }
    } catch (e) {
      Log.error("分页查询闪断帖已完成出现异常：$e");
    }
  }
  ...
  void _lookViePost(String postId) {
    CusRoute.push(context, UserVieHisPage(postId: postId));
  }
