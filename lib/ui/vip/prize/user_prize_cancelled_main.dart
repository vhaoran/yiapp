...
import 'package:yiapp/ui/vip/prize/user_prize_cancelled_page.dart';
...
class UserPrizeCancelledMain extends StatefulWidget {
  UserPrizeCancelledMain({Key key}) : super(key: key);

  @override
  _UserPrizeCancelledMainState createState() => _UserPrizeCancelledMainState();
}

class _UserPrizeCancelledMainState extends State<UserPrizeCancelledMain>
  ...
  _fetch() async {
    ...
    try {
      ...
      if (mounted) setState(() {});
      Log.info("已加载悬赏帖已取消个数 ${_l.length}");
    } catch (e) {
  ...
  void _lookPrizePost(String postId) {
    CusRoute.push(context, UserPrizeCancelledPage(postId: postId));
  }
